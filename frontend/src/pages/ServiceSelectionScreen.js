...
const ServiceSelectionScreen = () => {
  ...
  const { clientData, setSelectedService } = useBooking();
  ...
  const handleContinue = () => {
    if (!selectedServiceLocal) {
      showError('Por favor, selecciona un servicio');
      return;
    }

    showSuccess(`¡Excelente elección! ${selectedServiceLocal.name} seleccionado`);
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Button
            ...
            className="flex items-center gap-2 text-brand-amber-500 hover:text-brand-amber-400 font-bold"
          >
          ...
          {services.map((service) => (
            <Card
              ...
              className={`cursor-pointer transition-all duration-300 hover:scale-[1.02] relative overflow-hidden ${
                selectedServiceLocal?.id === service.id
              ...
              <div className="flex items-center justify-between relative z-10">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${
                      ...
                      <Scissors className={`w-6 h-6 transition-all duration-300 ${
                        selectedServiceLocal?.id === service.id ? 'text-white rotate-12' : 'text-zinc-300'
                      }`} />
                    </div>
                    <div>
                      <h3 className={`text-lg font-semibold transition-colors duration-300 ${
                        selectedServiceLocal?.id === service.id ? 'text-amber-100' : 'text-white'
                      }`}>
                        {service.name}
                      </h3>
                      {service.category && (
                  ...
                  <p className={`text-sm mb-3 transition-colors duration-300 ${
                    selectedServiceLocal?.id === service.id ? 'text-zinc-200' : 'text-zinc-400'
                  }`}>
                    {service.description}
                  </p>
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div className={`flex items-center gap-1 font-semibold transition-colors duration-300 ${
                      selectedServiceLocal?.id === service.id ? 'text-green-300' : 'text-green-400'
                    }`}>
                    ...
                    <div className={`flex items-center gap-1 transition-colors duration-300 ${
                      selectedServiceLocal?.id === service.id ? 'text-blue-300' : 'text-blue-400'
                    }`}>
              ...
              {service.isPopular && (
                <div className="absolute top-3 right-3 z-20">
                  <div className={`text-white text-xs px-3 py-1 rounded-full flex items-center gap-1 transition-all duration-300 ${
                    ...
                    <Star className={`w-3 h-3 ${selectedServiceLocal?.id === service.id ? 'animate-spin' : ''}`} />
              ...
              <div className="text-center relative">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-amber-300 font-semibold mb-1">
                    {selectedServiceLocal.name}
                  </p>
                  <p className="text-amber-400/80 text-sm mb-4">
                    {formatPrice(selectedServiceLocal.price)} • {formatDuration(selectedServiceLocal.duration)}
                  </p>
                  ...
                  >
                    ...
                    Continuar con {selectedServiceLocal.name}
